...
本报告按要求完成作业中的三项任务：第一项是在 Oxford-IIIT Pet Dataset 上微调 ImageNet 预训练 CNN 并完成宠物品种识别；第二项是在 VisDrone 数据集上训练 YOLOv8n 检测器，并在自采校园视频中完成多目标跟踪、遮挡分析和越线计数；第三项是从零搭建 U-Net，在 Oxford-IIIT Pet 三分类分割任务上比较 Cross-Entropy Loss、Dice Loss 与组合损失的 mIoU 表现。

实验代码按任务拆分在 \texttt{task1}、\texttt{task2}、\texttt{task3} 目录中。报告中的曲线与表格均来自仓库内保存的训练日志、结果 CSV、summary JSON 和输出图像。模型权重和视频按提交要求上传至网盘，链接见首页。

\subsection{SwanLab 可视化记录}

作业要求报告中包含 WandB 或 SwanLab 的可视化截图。由于本次实验的原始训练结果已经以 JSON、CSV 和图片文件保存在各任务目录中，本仓库使用 \path{report/log_to_swanlab.py} 将这些历史记录补录为 SwanLab local runs，再使用 \path{report/capture_swanlab_screenshots.py} 启动本地 SwanLab dashboard 并截图记录了任务 1 的分类训练曲线、任务 2 的 YOLOv8n 检测训练曲线，以及任务 3 三种损失函数配置下的分割训练曲线。

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{assets/swanlab_dashboard_overview.png}
\caption{SwanLab 本地看板项目页}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{assets/swanlab_dashboard_charts.png}
\caption{SwanLab 本地看板图表页：任务 1 与任务 2 的训练/验证曲线}
\end{figure}
...
本实验使用官方 \texttt{trainval} 部分按 8:2 划分为训练集和验证集，官方 \texttt{test} 部分作为测试集。所有输入图像统一 resize 到 $224\times224$，训练集使用随机水平翻转、随机旋转和颜色扰动，验证集和测试集仅进行尺寸调整、张量转换和 ImageNet 均值方差归一化。
...
超参数搜索过程中，我们尝试调整 backbone learning rate 与 classifier learning rate。搜索范围为 backbone learning rate $\{0.00005,0.0001,0.0005\}$，classifier learning rate $\{0.0005,0.001,0.005\}$，训练 15 epoch。
...
预训练消融实验使用完全相同的 ResNet-18 结构与训练设置。注意力机制实验在 ResNet-18 的 BasicBlock 中加入手写 SE-block，将 SE-block 放在第二个 BatchNorm 之后、残差连接相加之前：
...
任务 2 要求使用 VisDrone 无人机航拍检测数据集微调 YOLOv8 或同类单阶段检测模型，并将训练好的检测器用于 10--30 秒测试视频，实现逐帧检测、多目标跟踪、遮挡与 ID 跳变分析，以及基于 Tracking ID 的越线计数。本实验使用 YOLOv8n 作为基础模型，主要考虑到其参数量较小、训练和推理开销低，适合在本机显存环境中完成实验流程。

\subsection{训练设置与工程实现}

由于本地环境下多进程 DataLoader 在 Mosaic 数据增强中出现内存不足的情况，正式训练采用较保守设置：\texttt{batch=4}、\texttt{workers=0}。正式模型命名为 \texttt{visdrone\_yolov8n\_e50}，训练 50 epoch。
...
完成检测模型训练后，实验使用光华楼西侧道路视频作为测试视频。测试场景与 VisDrone 存在一定差异，其主要原因是我没有无人机，因此视频更接近地面视角；同时树叶、阴影等会对检测造成干扰。
...
根据任务要求，实验从跟踪视频中截取了连续遮挡片段。如下图所示，电动车目标在 Frame 586 被检测并分配 ID 29；随后几帧中检测框短暂消失，在遮挡车辆过去后重新出现，说明遮挡、背景干扰可能会引起置信度下降会导致目标丢失。

\begin{figure}[H]
\centering
\begin{subfigure}{0.32\textwidth}
  \centering
  \includegraphics[width=\textwidth]{../task2/outputs/frames/occlusion/frame_000586.jpg}
  \caption*{Frame 586}
\end{subfigure}
\hfill
\begin{subfigure}{0.32\textwidth}
  \centering
  \includegraphics[width=\textwidth]{../task2/outputs/frames/occlusion/frame_000587.jpg}
  \caption*{Frame 587}
\end{subfigure}
\hfill
\begin{subfigure}{0.32\textwidth}
  \centering
  \includegraphics[width=\textwidth]{../task2/outputs/frames/occlusion/frame_000588.jpg}
  \caption*{Frame 588}
\end{subfigure}
...
\begin{subfigure}{0.32\textwidth}
  \centering
  \includegraphics[width=\textwidth]{../task2/outputs/frames/occlusion/frame_000589.jpg}
  \caption*{Frame 589}
\end{subfigure}
\hfill
\begin{subfigure}{0.32\textwidth}
  \centering
  \includegraphics[width=\textwidth]{../task2/outputs/frames/occlusion/frame_000590.jpg}
  \caption*{Frame 590}
\end{subfigure}
\caption{任务 2 遮挡片段连续帧可视化。}
\end{figure}

该片段中没有发生ID switch，但检测器对遮挡目标召回不足，导致目标框短时中断。可能的原理或原因：多目标跟踪依赖检测结果、运动预测和帧间匹配；当检测框消失时，跟踪器只能依赖短期轨迹缓存维持目标，遮挡较短时可能恢复原有 ID，遮挡较长或相似目标交汇时则更容易发生 ID switch。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{assets/line_occusion.jpg}
\caption{任务 2 越线计数关键帧截取。}
\end{figure}

实际测试视频中，人工观察约有 3 个目标穿过虚拟线，而程序统计结果为 1。偏差主要来自检测和跟踪不稳定：目标靠近虚拟线时若发生漏检、类别突变或 ID 轨迹中断，程序无法获得完整的线前和线后位置，越线逻辑就不会触发。
...
\end{lstlisting}
模型测试中，输入形状 $[2,3,256,256]$ 时输出为 $[2,3,256,256]$，满足逐像素三分类要求。

\subsection{Cross-Entropy Loss 实现}

仓库中的实现位于 \path{task3/losses/dice_loss.py}。虽然文件名为 \texttt{dice\_loss.py}，其中的 \texttt{build\_loss} 同时负责构建 CE、Dice 和 CE + Dice 三种损失。当配置文件中的 \texttt{train.loss\_type} 为 \texttt{ce} 时，直接返回 PyTorch 的 \texttt{nn.CrossEntropyLoss()}：
...
训练流程位于 \path{task3/train.py}，三种损失函数都通过同一个接口调用。模型先得到 logits，然后将 logits 与整型 mask 一起送入 criterion：
...
因此 CE 组实验对应的实际计算就是 \texttt{CrossEntropyLoss(logits, masks)}。CE 的优势是逐像素分类梯度稳定、实现简单；不足是对像素类别面积不均衡较敏感，尤其边界类像素较少时，模型可能更偏向大面积类别，因此后续进一步比较 Dice Loss 与 CE + Dice 组合损失。
...
任务 1 中，预训练 ResNet-18 在 Oxford-IIIT Pet 细粒度分类上显著优于随机初始化模型，说明迁移学习对中小规模视觉任务非常关键。超参数搜索表明 backbone learning rate 不宜过大。SE-block 完成了注意力机制引入要求，但在当前设置下未超过 Baseline。

任务 2 中，YOLOv8n 在 VisDrone 上完成 50 epoch 微调，获得整体 Precision 0.412、Recall 0.318、mAP50 0.287、mAP50-95 0.162。自采视频中能够输出检测框、类别与 Tracking ID，并实现遮挡分析和越线计数，但由各种原因实际效果不佳。

任务 3 中，手写 U-Net 完成了无预训练语义分割训练，并手动实现 Dice Loss。CE + Dice 组合损失取得最高验证 mIoU 0.770272，验证了 Dice Loss 在类别不均衡分割任务中的价值，说明组合损失能兼顾像素分类准确性和区域重叠质量。

本次作业完整完成了作业要求的三类计算机视觉任务，代码、训练结果、可视化图、模型权重与复现说明均已整理到仓库与网盘中。
