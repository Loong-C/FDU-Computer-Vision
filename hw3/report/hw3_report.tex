...
\begin{titlepage}
  ...
  \begin{tabularx}{0.94\textwidth}{>{\bfseries}p{3.0cm}Y}
    ...
    \midrule
    题目一代码仓库 &
    \href{https://github.com/Loong-C/FDU-Computer-Vision/tree/main/hw3/task1}{github.com/Loong-C/FDU-Computer-Vision（main/hw3）} \\
    题目一模型权重 &
    ...
    题目二模型权重 &
    \href{\clouddrive}{Google Drive：hw3-task2-act-*.pt} \\
    题目二 SwanLab &
  ...
  \begin{minipage}{0.90\textwidth}
    ...
    公开下载地址；题目一资产包与漫游视频提供 GitHub Release，题目二权重保存在
    Google Drive 镜像目录中。
  \end{minipage}
  \vspace{0.7cm}
\end{titlepage}
...
\begin{table*}[t]
\centering
\caption{HW3 明确要求与本报告对应位置。}
\label{tab:requirement-checklist}
\small
\begin{tabularx}{0.98\textwidth}{p{2.45cm}p{4.65cm}Y}
\toprule
类别 & 作业要求 & 报告与交付位置 \\
\midrule
全局信息 &
首页标明姓名、学号、组队与具体分工；提供 GitHub 与模型权重下载地址 &
首页已列出陈家龙、24300980041、单人完成与独立完成；首页和表~\ref{tab:deliverables}
列出统一 GitHub 仓库、GitHub Release、Google Drive 与权重 SHA256 \\
题目一资产 A &
手机采集真实物体，多视角/COLMAP 位姿，使用 2DGS 重建 &
表~\ref{tab:task1-data} 写明 63 张手机照片与 34 张注册图；2.2 节给出 COLMAP 与 2DGS 方法；
表~\ref{tab:2dgs-results} 和图~\ref{fig:2dgs-metrics} 给出验证指标 \\
题目一资产 B/C &
B 使用 threestudio、SDS 与文本 Prompt；C 使用去背景单图与 Magic123 &
2.3 节给出 DreamFusion/SDS Prompt 与导出规模；2.4 节给出去背景、MiDaS、Magic123 coarse/fine
流程；图~\ref{fig:aigc-curves} 展示 AIGC 优化曲线 \\
背景与融合 &
开源 3D 场景使用 2DGS 重建；将 A/B/C 按合理比例插入并生成多视角漫游视频 &
选择 Mip-NeRF 360 \code{counter}；2.5 节说明 Mesh 统一、Blender 位姿与相机路径；
图~\ref{fig:fusion-pipeline} 和图~\ref{fig:fusion-runtime} 展示流程与融合帧，视频链接见首页 \\
题目一分析 &
比较多视角重建、文本生成、单图生成在几何准确度、纹理细节、计算耗时上的差异；说明异构表达统一方式 &
图~\ref{fig:task1-triviews} 展示三种方法的三视图；2.7.2 节逐项分析几何、纹理与耗时；
表~\ref{tab:task1-analysis} 汇总边界，2.5 节说明由 2DGS/隐式场统一到 Mesh 后融合渲染 \\
题目二训练与测试 &
使用 LeRobot ACT 分别训练 B-only 与 A+B+C；架构和超参数一致；比较训练收敛、D 环境 zero-shot &
表~\ref{tab:task2-hyper} 给出相同 ACT 超参数；图~\ref{fig:act-curves} 展示 Action L1 与验证损失；
表~\ref{tab:act-results}、图~\ref{fig:act-zeroshot} 与表~\ref{tab:rollout-results} 给出 D 动作误差和
simulator 成功率 \\
提交材料 &
报告需含背景、数据集、方法、结果、深度分析、SwanLab/WandB 图表、超参数表、代码 README 与权重 &
正文按任务组织；表~\ref{tab:swanlab} 列出 SwanLab 项目和各运行链接；代码位于
\code{hw3/task1} 与 \code{hw3/task2}，README 含环境、数据、训练与测试命令 \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{table}[H]
\centering
\caption{公开交付物。题目一使用 FDU 仓库 GitHub Release；题目二权重使用 Google Drive 镜像目录。}
...
题目二权重镜像目录 &
-- &
\href{\clouddrive}{Google Drive：hw3-task2-act-b-only-best.pt 与 hw3-task2-act-abc-joint-best.pt} \\
Google Drive 镜像目录 &
-- &
