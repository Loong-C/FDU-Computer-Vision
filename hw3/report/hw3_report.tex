...
\newcommand{\tasktworelease}{https://github.com/Loong-C/computer-vision-hw3-task2-calvin-act/releases/tag/formal-partial-v1}
...
\begin{titlepage}
  ...
  \begin{tabularx}{0.88\textwidth}{>{\bfseries}p{3.1cm}Y}
    ...
    学号 & 24300980041 \\
    组队情况 & 单人完成 \\
    具体分工 & 独立完成两道题目的环境配置、数据准备、训练与推理、实验记录、结果分析、代码整理及报告撰写。 \\
    报告日期 & 2026 年 6 月 2 日 \\
    \bottomrule
  \end{tabularx}
  ...
  \begin{tabularx}{0.94\textwidth}{>{\bfseries}p{3.0cm}Y}
    ...
    \midrule
    题目一代码仓库 &
    \href{https://github.com/Loong-C/FDU-Computer-Vision/tree/hw3/hw3/task1}{github.com/Loong-C/FDU-Computer-Vision（hw3 分支）} \\
    题目一模型权重 &
    \href{\taskoneweights}{GitHub Release：cv-hw3-task1-best-weights.tar.gz} \\
    题目一漫游视频 &
    \href{\taskonevideo}{GitHub Release：task1-walkthrough.mp4} \\
    题目二代码仓库 &
    \href{https://github.com/Loong-C/computer-vision-hw3-task2-calvin-act}{github.com/Loong-C/computer-vision-hw3-task2-calvin-act} \\
    题目二模型权重 &
    \href{\tasktworelease}{GitHub Release：formal-partial-v1} \\
    题目二 SwanLab &
  ...
  \begin{minipage}{0.90\textwidth}
    ...
    GitHub Release 下载地址，并在 Google Drive 中保留镜像。
  \end{minipage}
  \vspace{0.7cm}
\end{titlepage}
...
环境 D 上比较零样本动作误差。实验表明：多视角重建在几何可靠性上最强，文本到 3D 输入成本
...
SwanLab 本地运行；题目二在 SwanLab 中记录 Action L1、KL、验证损失和环境 D 动作误差，同时
保留 CSV 备份。报告中的曲线均由记录指标生成。
...
横向比较可见，输入信息越充分，跨视角一致性通常越可靠：多视角路线最适合真实对象还原；文本路线
适合快速扩充语义正确的虚拟资产；单图路线能够保留特定对象的正面身份信息，但不可见区域仍有较大
不确定性。三视图分析也说明，仅检查一张最优正面预览容易高估 AIGC 资产的几何质量。
...
正面纹理保持较好，侧后纹理的真实性较弱 &
采集简单但优化最慢；本实验 coarse + fine 合计 33039.30 s &
...
用途 & 选取帧数 & 处理后样本数 \\
\midrule
A+B+C 训练源数据 & 2304 & 691 训练 / 77 验证 \\
B-only 训练源数据 & 768 & 230 训练 / 26 验证 \\
D 环境 zero-shot 评估 & 768 & 768 \\
\bottomrule
\end{tabular}}
\end{table}

训练数据使用 stride 3 与 90/10 训练验证划分。B-only 与 A+B+C 模型的样本数量不同，
联合训练模型能够接触三种场景的视觉多样性；二者的网络、优化器、步数和评估脚本
完全一致，因此差异可以归因于训练环境分布变化。
...
\begin{figure}[H]
  ...
  \caption{环境 D 上的 zero-shot 动作误差。与 B-only 相比，A+B+C 联合训练使 first-action
...
题目二使用官方 CALVIN 帧的均匀抽样子集，尚未覆盖完整的 517 GB / 166 GB 数据归档。评估指标
采用环境 D 上的动作 MAE，未运行 Linux CALVIN 模拟器 rollout 成功率。该指标符合题目允许的
替代方案，可以衡量离线动作接近程度，但无法完整反映长时间闭环任务成功率。因此，本实验结论
限定在报告所述资源约束协议内：多环境联合训练提升了未见环境 D 的离线动作泛化能力。
...
题目一和题目二仓库根目录均提供 README、环境依赖、数据准备说明、训练命令与测试命令。下面仅列出
主入口；完整命令、兼容性说明与缓存设置见对应仓库 README。

\begin{lstlisting}[language=bash,caption={题目一主要复现入口。}]
# Object A and background: COLMAP + 2DGS
...
\begin{table*}[t]
\centering
\caption{公开交付物。GitHub Release 提供直接下载，Google Drive 文件夹提供镜像。}
...
题目二 Release 页面 &
-- &
\url{https://github.com/Loong-C/computer-vision-hw3-task2-calvin-act/releases/tag/formal-partial-v1} \\
Google Drive 镜像目录 &
-- &
...
对照：A+B+C 联合训练虽然增加了拟合难度，却显著降低了未见环境 D 上的动作误差。该现象支持
视觉多样性训练与动作分块有助于提高跨环境鲁棒性；chunk horizon 增大后，不确定性累积仍需要
重点关注。
