...
\begin{titlepage}
  ...
  \begin{tabularx}{0.94\textwidth}{>{\bfseries}p{3.0cm}Y}
    ...
    SwanLab &
    \href{https://swanlab.cn/@Linkukai}{swanlab.cn/@Linkukai} \\
    网盘镜像 &
...
最低，单图到 3D 则在真实外观保持与采集成本之间折中；ACT 的多环境训练具有略高的序列验证损失，
并使环境 D 的 first-action MAE 和 chunk-action MAE 分别下降 22.0\% 和 17.2\%。
...
\midrule
A+B+C 训练源数据 & 2304 & 634.01 MiB & 672 训练 / 96 验证 \\
B-only 训练源数据 & 768 & 包含于上行 & 224 训练 / 32 验证 \\
...
\code{.cache} 索引文件，用于定位远程 ZIP 中的条目。每个环境包含 16 个连续的 48 帧窗口，
stride 3 后每个窗口产生 16 个训练样本。划分时以连续窗口为单位，并在 A、B、C 内分别固定留出
2 个窗口作为验证集；同一窗口内的样本始终归入同一集合。20 步动作块仅在所属连续窗口内取监督，
经运行时审计，训练集与验证集使用的动作帧交集为 0。B-only 与 A+B+C 的 B 环境验证窗口一致，
二者的网络、优化器、步数和评估脚本也完全一致。
...
\small
\begin{tabularx}{\columnwidth}{p{2.25cm}p{1.45cm}Y}
\toprule
项目 & 取值 & 说明 \\
\midrule
视觉输入 & $128\times128$ & ResNet-18，不加载预训练 backbone 权重 \\
Transformer & 256 维 & 8 heads，feedforward 维度 1024 \\
编码器 / 解码器 & 3 / 1 层 & VAE 编码器 2 层，latent 维度 32 \\
状态 / 动作 & 15 / 7 维 & 动作块长度 20，KL 权重 10 \\
优化器 & AdamW & 学习率 $1\times10^{-5}$，权重衰减 $1\times10^{-4}$ \\
batch size & 4 & AMP 开启，梯度裁剪范数 10 \\
训练与验证 & 5000 步 & 每 250 步验证 30 batches \\
设备 & RTX 4060 Ti & 8 GB 显存，Windows 本地 CUDA 环境 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure*}[t]
  ...
  \caption{由 SwanLab 记录指标导出的 ACT 训练 Action L1 与连续窗口验证曲线。训练损失持续下降；
  验证损失在早期达到最低值后出现波动，反映了小规模训练窗口上的过拟合。}
...
\begin{tabular}{lrrr}
\toprule
训练环境 & 最佳序列验证 L1 & D first-action MAE & D chunk MAE \\
\midrule
B-only & \textbf{0.505717} & 0.253965 & 0.268460 \\
A+B+C & 0.561016 & \textbf{0.198150} & \textbf{0.222258} \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{figure}[H]
  ...
  MAE 下降 22.0\%，chunk-action MAE 下降 17.2\%。}
...
图~\ref{fig:act-curves} 中，两个模型的训练 Action L1 均随优化推进而下降，连续窗口验证损失则在
早期达到最低值：B-only 在第 250 步取得 0.505717，A+B+C 在第 500 步取得 0.561016。
后续训练损失继续降低，验证曲线整体升高并伴随波动，说明当前 14 个 B 训练窗口和 42 个 A+B+C
训练窗口仍不足以支撑 5000 步持续拟合。正式权重按最低验证损失选取。

B-only 验证集只含 B 环境窗口，A+B+C 验证集同时覆盖 A、B、C，因此两项验证 L1 的绝对值
反映了不同难度的数据分布。两种策略的跨环境能力使用同一组 D 环境帧评估。A+B+C 的
first-action MAE 从 0.253965 降至 0.198150，改善 22.0\%；chunk-action MAE 从
0.268460 降至 0.222258，改善 17.2\%。动作块指标的改善幅度较小，说明视觉分布偏移仍会在
长 horizon 预测中累积不确定性。两项 D 指标同步下降，表明多环境视觉训练同时改善了立即决策
和整段 20 步预测。
...
但 3 条五任务序列的 SR@1--SR@5 均为 0。离线 MAE 衡量给定单帧观测时预测动作与专家动作的
平均距离，数值下降说明模型在 D 环境的局部
...
题目二使用 846.8 MiB 的 CALVIN 均匀抽样帧，未覆盖完整的 517 GB / 166 GB 数据归档。
simulator rollout 仅评估 3 条五任务序列，成功率用于验证部署链路，不能代表充分训练后的最终能力。
离线 action-error 衡量单帧动作与专家动作的接近程度；rollout success 进一步考察闭环执行、
误差累积和任务链恢复。本实验据此将结论限定在当前资源协议：多环境训练改善了环境 D 的离线动作泛化，
3 条小规模 rollout 中尚无成功子任务。
...
题目二 B-only 训练 &
\url{https://swanlab.cn/@Linkukai/hw3-calvin-act/runs/5604rx1q61tfydytkh9eo} \\
题目二 A+B+C 训练 &
\url{https://swanlab.cn/@Linkukai/hw3-calvin-act/runs/42di3m9zyj2321v11e3xb} \\
题目二 B-only $\rightarrow$ D 评估 &
\url{https://swanlab.cn/@Linkukai/hw3-calvin-act/runs/dq0p2cm8j0ypczffatbj6} \\
题目二 A+B+C $\rightarrow$ D 评估 &
\url{https://swanlab.cn/@Linkukai/hw3-calvin-act/runs/6dbyoupmdn0c9loq5p9dv} \\
题目二 B-only $\rightarrow$ D rollout &
\url{https://swanlab.cn/@Linkukai/hw3-calvin-act/runs/yeltzn7w84t477i0pyme1} \\
题目二 A+B+C $\rightarrow$ D rollout &
\url{https://swanlab.cn/@Linkukai/hw3-calvin-act/runs/05wghj2bt442shmdzxijj} \\
\bottomrule
\end{tabularx}
\end{table}
...
\code{49ad38cb15b38fa1ae208caac70da0e41536aa3dadcd6e7408a58647bed06ce5} \\
题目二 A+B+C 最佳权重 &
196370932 bytes &
\href{\tasktwoabcweights}{GitHub Release 下载}；
\code{7a1ef8617b7f741c0dd8e73b3a6d6c23b6d03381d543f13e95d7150b61832c5b} \\
题目二权重 Release &
-- &
...
对照。A+B+C 联合训练在独立连续窗口上的最佳验证损失略高，并降低了未见环境 D 上的离线动作误差。进一步的
